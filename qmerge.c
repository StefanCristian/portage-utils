		...
		const char     *replacing)
{
	...
	xasprintf(&script,
		...
		"    3) echo \"${c[2]}\" ;;\n"
		"    1-2) echo \"${c[0]}.${c[1]}\" ;;\n"
		"    1-3) echo \"${c[0]}.${c[1]}.${c[2]}\" ;;\n"
		"    *) echo \"$v\" ;;\n"
		"  esac\n"
		"}\n"
		"ver_rs() { echo \"${2:-${PV}}\"; }\n"
		/* Library directory functions */
		"get_libdir() {\n"
		"  local abi=${ABI:-default}\n"
		"  case ${abi} in\n"
		"    x86|amd64_x32) echo lib32 ;;\n"
		"    amd64|x32) echo lib64 ;;\n"
		"    *) echo lib ;;\n"
		"  esac\n"
		"}\n"
		"get_abi_var() { :; }\n"
		"get_abi_LIBDIR() { get_libdir; }\n"
		/* Multilib functions */
		...
		"dodir() { local d; for d in \"$@\"; do d=\"${d#/}\"; mkdir -p \"${ED}/${d}\"; done; }\n"
		"keepdir() { local d; for d in \"$@\"; do d=\"${d#/}\"; dodir \"$d\" && touch \"${ED}/${d}/.keep_${CATEGORY}_${PN}-${SLOT%%/*}\"; done; }\n"
